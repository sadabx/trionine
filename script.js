(function () {
        ...
        async function renderProjects() {
          ...
          //  Read-Only PAT Token
          const GITHUB_TOKEN = "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"; 
          ...
          for (const site of coreWebsites) {
            try {
              const response = await fetch(`https://api.github.com/repos/${site.repoPath}`, {
                headers: {
              ...
              const diffDays = Math.floor(Math.abs(new Date() - updatedDate) / (1000 * 60 * 60 * 24));
              const updatedText = diffDays === 0 ? "today" : diffDays === 1 ? "yesterday" : `${diffDays} days ago`;

              // Map languages to colors
              let langClass = repo.language === "JavaScript" ? "lang-js" : repo.language === "CSS" ? "lang-css" : "";

              html += `
                <div class="project-card fade-in appear">
                  <div class="card-image"><img src="${site.img}" alt="${repo.name}"/></div>
                  <div class="card-content">
                    <h3><a href="${site.link}" target="_blank">${repo.name.replace(/-/g, ' ')}</a></h3>
                    <p>${repo.description || "No description provided."}</p>
                    <div class="tags">
                      ${repo.language ? `<span class="tag ${langClass}"><span class="lang-dot"></span> ${repo.language}</span>` : ""}
          ...
          // 2. Fetch standard Public Repos (Without the token)
          try {
            ...
            filteredRepos.forEach((repo) => {
              let langClass = repo.language === "JavaScript" ? "lang-js" : repo.language === "C++" ? "lang-cpp" : "";
              ...
              html += `
                <div class="project-card fade-in appear">
                  <div class="card-content">
                    ...
                    <div class="project-links">
                      <a href="${repo.html_url}" class="btn btn-outline" target="_blank"><i class="fab fa-github"></i> Code</a>
                      ${repo.homepage ? `<a href="${repo.homepage}" class="btn btn-outline" target="_blank"><i class="fas fa-external-link-alt"></i> Demo</a>` : ""}
                    </div>
                  </div>
                </div>`;
            });
