...
function setupFormHandler() {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (!isLoggedIn) {
      showFormMessage('You need to be logged in to share resources.', 'error');
      return;
    }
    
    if (!supabase) {
      showFormMessage('Database connection not available. Please refresh the page.', 'error');
      return;
    }
    ...
    const resourceData = {
      ...
      download_url: document.getElementById('download-link').value,
      shared_by: currentUser.user_metadata?.full_name || currentUser.email,
    ...
    if (error) {
      ...
      showFormMessage('Error submitting resource: ' + error.message, 'error');
    } else {
      console.log('✅ Resource submitted successfully!');
      showFormMessage('Thank you for contributing to our open source library! Your resource has been added.', 'success');
      form.reset();
      loadResources();
    }
...
// Setup form visibility based on auth state
function setupFormVisibility() {
  ...
  checkUserAuth().then(isLoggedIn => {
    if (isLoggedIn) {
      ...
      if (resourceForm) resourceForm.style.display = 'block';
    } else {
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Wait a bit for the supabase config to load
  setTimeout(async () => {
